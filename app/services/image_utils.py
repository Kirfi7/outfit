...
def open_image(image_bytes: bytes) -> Image.Image:
    """
    Открывает любые входные bytes (jpg/png/heic/...) в PIL Image, фиксит EXIF orientation, приводит к RGB.
    """
    try:
...
def make_center_crop(img: Image.Image, crop_w: float = 0.62, crop_h: float = 0.62) -> Image.Image:
    """
    Центровой crop: обычно нужен чтобы выделить область принта.
    """
    w, h = img.size
    cw = max(1, int(w * crop_w))
    ch = max(1, int(h * crop_h))
    left = max(0, (w - cw) // 2)
    top = max(0, (h - ch) // 2)
